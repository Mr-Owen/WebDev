...
//   note:之前是直接加减而不是取绝对图像位置，容易在点击圆点和箭头之间出现慢一步的情况，故这里改为求绝对图像位置
  ...
  return {
    ...
    animate: (num, tg) => {
      ...
      switch (digit) {
        case 0:
          ...
          img_index = -parseInt(list.style.left, 10) / img_width + 1; // 取left计算后的值，所以不能取digit,+1防止负0出现
          break;
    ...
    point: (item) => {
      if (typeof item !== "undefined") {
        img_index = item; // 最好在函数内修改私有变量
      }
      for (let i = 0; i < img_len; i++) {
        span[i].classList.remove("on");
      }
      span[img_index - 1].classList.add("on");
    }
  };
})();
